...
$(document).ready(() => {
    ...
    */
    $("#purge-cart").click( function() {
        $.post("<?= BASE_URL . "store/purge-cart" ?>", function(data) { $("#cart").html(data); });
    });

    $(".update-cart").change( function() {
        $.post("<?= BASE_URL . "store/update-cart" ?>", { id: $(this).data("id"), quantity: $(this).val()}, function(data) {$("#cart").html(data);});
    });
});
</script>
